...
2. **Slash-command check.** Built-in commands (`/help`, `/skills`, `/mcp_servers`, `/compact`, `/jobs`, `/show`, `/comment`, `/unblock`, `/exit`) short-circuit before the LLM runs. User-defined `.command.md` commands are expanded into a normal prompt. See [Slash commands](/cli/slash-commands).
...
- **Long-running work inside a single turn.** A `chat`/`run` turn is request/response. For durable, multi-step, or scheduled background work, the runtime has a separate **job lane** — see [Jobs & Scheduling](/jobs/overview).
- **State outside the session.** Skills persist whatever they want in their own folder (typically a JSON file under `_data/`). The runtime owns the session store, the cache, the cost tracker, and (for background work) the job board.
