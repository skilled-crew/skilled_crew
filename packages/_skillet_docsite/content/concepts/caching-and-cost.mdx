...
Every OpenAI request the runtime makes flows through two SQLite-backed middlewares: a **response cache** and a **per-call cost tracker**. Both ship out of the box and are safe to delete at any time. They live in different [`SkilletPaths`](/api/classes/SkilletPaths) roles — the cache under the disposable **cache** directory, the cost tracker under the persistent **state** directory (see [where these files live](/concepts/sessions-and-logs#where-these-files-live)). The paths below use the development layout, relative to the repo root.
...
# equivalent to: rm -f ../../.skilled-agent/cache/.openai_cache.sqlite
```
...
npx --package=openai-cost openai_cost_pp \
  ...
  -k userId,skilletId \
  -i ../../.skilled-agent/state/.openai_cost_tracker.sqlite \
  -w
```
...
# equivalent to: rm -f ../../.skilled-agent/state/.openai_cost_tracker.sqlite
```
